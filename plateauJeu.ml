...
let color_circle2 color raw line =
  color_circle color (w/18 + raw*w/9) (h/9 + h/18 + line*h/9);;
...
let gameboard current_game =
  ...
  (*utilisation bouton New Part et du quadrillage*)
  let rec part player1 player2 =
    ...
    if button_down() then
      (
        ...
        then
          (
            let raw = pos_x/(w/9) in
            if (current_game.tab.(raw-1).(5) = Empty) then
              (
