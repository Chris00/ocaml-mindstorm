...
let rec ab col game alpha beta mode depth heuristic =
  let z = Gamemem.fast_try_to_win game in
    if z <> -1 then if mode = Max then (infinity, z) else (neg_infinity, z)
    else if Gamemem.get_game_result game = Gamemem.DRAW then (0., col)
