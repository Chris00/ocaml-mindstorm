...
(*création d'un jeu -> on initialise le tableau a Empty
  et la liste d'événement est vide*)
let make () = { tab = Array.init 7 (fun i -> (Array.make 6 Empty));
                tab_line = Array.init 7 (fun i -> Array.init 6
...
let move current_game j p =
  ...
  if (!i<>n-1) then
    (
      ...
      (*on remplit le tab_line_piece du pion courant et on modifie les tab
        des pions de meme couleurs qui lui sont alignés*)
      ...
      (*on modifie les données des cases adjacentes se trouvant dans
        une meme ligne que le pion courant*)
...
let new_part current_game =
  ...
  current_game.tab_line <- Array.init 7 (fun i -> Array.init 6
